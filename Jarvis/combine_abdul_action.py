...
with open('predictions.csv', mode='w', newline='') as file:
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        while cap.isOpened():
            ...
            # Append predictions and timestamps to CSV file
...
# JARVIS Command Execution Script
import csv
import os
import time

# Define the mapping of actions to commands
commands = {
    "hello": "start chrome",
    "thanks": "start notepad",
    "iloveyou": "start wmplayer"
}
...
def main():
    ...
    # Initialize a set to keep track of processed timestamps
    ...
    while True:
        try:
            with open(csv_file, mode='r') as file:
                ...
                for row in reader:
                    timestamp, predicted_action = row
                    timestamp = float(timestamp)

                    # Check if the action has already been processed
                    if timestamp not in processed_timestamps:
                        execute_command(predicted_action)
                        processed_timestamps.add(timestamp)

            time.sleep(5)  # Check for new actions every 5 seconds
